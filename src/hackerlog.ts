...
export default class Hackerlog {
  ...
  private extension = this.vscode.extensions.getExtension('Hackerlog.hackerlog').packageJSON;
  ...
  public initialize(): void {
    ...
    this.dependencies.checkAndInstall(() => {
      ...
      this.options.getSetting('settings', 'statusBarIcon', (err, val) => {
  ...
  public promptForApiKey(): void {
    this.options.getSetting('settings', 'api_key', (err, defaultVal) => {
      ...
      let promptOptions = {
        prompt: 'Hackerlog API Key',
        placeHolder: 'Enter your api key from hackerlog.io/me',
        value: defaultVal,
        ignoreFocusOut: true,
  ...
  public promptStatusBarIcon(): void {
    this.options.getSetting('settings', 'statusBarIcon', (err, defaultVal) => {
  ...
  private validateKey(key: string): string {
    const err = 'Invalid api key... check https://hackerlog.io/me for your key.';
    if (!key) return err;
    const re = new RegExp(
  ...
  private hasApiKey(callback: (boolean) => void): void {
    this.options.getSetting('settings', 'api_key', (error, editorKey) => {
      ...
      if (hasApiKey) {
        this.dependencies.getPythonLocation(pythonBinary => {
          if (pythonBinary) {
            ...
            let user_agent =
              'vscode/' + this.vscode.version + ' vscode-Hackerlog/' + this.extension.version;
